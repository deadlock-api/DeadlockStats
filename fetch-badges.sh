...
echo "Fetching rank list and downloading images..."
...
curl -sL "$API_URL" | \
      jq -r '.[] |
        ...
        .images |
        to_entries[] |
        select(.key | test("small_subrank[1-6]_" + "'${IMAGE_EXT}'")) |
        "\($tier)\(.key | capture("subrank(?<subtier>[1-6])") | .subtier) \(.value)"' | \
      while read -r rank_id image_url; do
        dest_file="$DEST_DIR/$rank_id.$IMAGE_EXT"
...
# Special Download the unranked badge
curl -Lsf -o $DEST_DIR/0.$IMAGE_EXT "https://assets-bucket.deadlock-api.com/assets-api-res/images/ranks/rank0/badge_sm.webp"

echo -e "${COLOR_GREEN}All rank images have been downloaded and validated.${COLOR_NONE}"