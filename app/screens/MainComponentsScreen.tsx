...
import { ListView, type ListViewRef } from "@/components/ui/ListView";
import { Screen } from "@/components/ui/Screen";
import { Text } from "@/components/ui/Text";
import { isRTL, type TxKeyPath } from "@/i18n";
import { translate } from "@/i18n/translate";
import type { MainTabParamList, MainTabScreenProps } from "@/navigators/MainNavigator";
...
const WebListItem: FC<DemoListItem> = ({ item, sectionIndex }) => {
  ...
  return (
    <View>
      <Link screen="MainComponents" params={{ queryIndex: sectionSlug }} style={themed($menuContainer)}>
...
export const MainComponentsScreen: FC<MainTabScreenProps<"MainComponents">> = function DemoShowroomScreen(_props) {
  const [open, setOpen] = useState(false);
  const timeout = useRef<ReturnType<typeof setTimeout>>(null);
  const listRef = useRef<SectionList>(null);
  const menuRef = useRef<ListViewRef<DemoListItem["item"]>>(null);
  const route = useRoute<RouteProp<MainTabParamList, "MainComponents">>();
  ...
  useEffect(() => {
    if (params !== undefined && Object.keys(params).length > 0) {
      ...
      if (params.itemIndex) {
        try {
          findItemIndex = demoValues[findSectionIndex].data({ themed, theme }).findIndex((u: ReactElement) => {
